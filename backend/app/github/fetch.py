...
def sync_issue_comments(client: GitHubClient, repo: str, issue_number: int) -> list[dict]:
    """Helper to sync comments on a single issue."""
    cache_root = get_cache_dir(repo)
    comments_cache = cache_root / "comments" / f"issue_{issue_number}_comments.json"
    raw_comments, _ = get_cached_or_fetch(
        comments_cache,
        lambda: client.list_comments(repo, issue_number, max_items=100),
    )
    collab_cache = cache_root / "collaborators" / "collaborators.json"
    raw_collabs, _ = get_cached_or_fetch(
        collab_cache,
        lambda: client.list_repo_collaborators(repo, max_items=100),
    )
    maintainer_logins = {
    ...
    normalized = [normalize_comment(c, maintainer_logins) for c in (raw_comments or [])]
